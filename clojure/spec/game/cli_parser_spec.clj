...
(describe "cli-parser"
  ...
  (it "interprets arguments"
      (should-have-msg-and-options (interpret (parse-args ["-h"]))
                                   (usage summ) (:options default-args-with-help))
      (should-have-msg-and-options (interpret (parse-args ["foo"]))
                                   (usage summ) default-options)
      (should-have-msg-and-options (interpret (parse-args ["-t 4"]))
                                   (errors-msg (invalid "-t 4" token-msg)) default-options)
      (should-have-msg-and-options (interpret (parse-args ["-f 4"]))
                                   (errors-msg (invalid "-f 4" non-numeric)) default-options)
      (should-have-msg-and-options (interpret (parse-args ["-t 3"]))
